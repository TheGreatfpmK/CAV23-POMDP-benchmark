...
Given the nature of the experiments, their outcome heavily depends on the timing, so the produced tables and figures will be different, although the underlying qualitative comparison of the approaches should be preserved. The original results were obtained on a PC equipped with i5-12600k @4.9GHz and 64GB of RAM. The experiments can be run on much more modestly equiped PCs/laptops, although it might happen that some more demanding experiments will timeout or run out of memory. In such a case, the corresponding table cell will contain TO/MO. If you obtain this result for Storm or SAYNT, it most likely means MO (that lead to a drastic slowdown and subsequent timeout), so there is nothing you can do about it. If you see TO/MO result for PAYNT, this can only mean timeout since PAYNT is very memory-friendly. If you wish to re-run the experiment with a larger timeout, you may remove the log files for the problematic benchmark, increase the `timeout_multiplier` variable at the top of the `experiments/experiments.py` file (e.g. setting the value to 2 will double the timeout) and then re-run the evaluating script; the script will only re-run experiments whose log files you had removed. Once you're satisfied with the result, don't forget to reset the `timeout_multiplier` variable.
